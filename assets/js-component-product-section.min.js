...
class ProductSection extends HTMLElement {
  ...
  #changeHTML(html) {
    ...
    this.querySelector(`[${attributes.json}]`).innerHTML = $newElement.querySelector(`[${attributes.json}]`).innerHTML;

    const $$prevAreas = Array.from(this.querySelectorAll(`[${ attributes.area }]`));
    const $$newAreas = Array.from($newElement.querySelectorAll(`[${ attributes.area }]`));

    if ($$prevAreas.length < $$newAreas.length) {
      console.warn(`Previous ${ attributes.area } elements don't match the new received ones`);
      return this.innerHTML = newElementHtml;
    }

    $$prevAreas.forEach($prevArea => {
      const areaId = $prevArea.getAttribute(attributes.area);
      const newAreaIndex = $$newAreas.findIndex(el => el.getAttribute(attributes.area) === areaId);
      if (newAreaIndex === -1) {
        console.warn(`Previous ${ attributes.area } elements don't match the new received ones`);
        $prevArea.remove();
        return;
