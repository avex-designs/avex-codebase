...
class ProductSection extends HTMLElement {
  ...
  #hydrate() {
    ...
    try {
      ...
      if (!$jsonScript)
        throw new Error(`The mandatory ${attributes.json} element is not found`);
      this.#data = JSON.parse($jsonScript.innerHTML);

      // set the #optionsHierarchy to match the options order
      this.#optionsHierarchy = this.#data.product.options.map((option, index) => index);

      if (!('requestURL' in this.#data.section) || !('sectionId' in this.#data.section))
  ...
  setVariantId(variantId) {
    ...
    if (variantId > 0) {
      const variant = this.#data.product.variants.find(variant => variant.id === variantId);
      if (!variant)
        throw new Error(`The variant "${ variantId }" is not found in the product data object`);
  ...
  #changeState({ variantId, optionsValues }) {
    ...
    const areOptionsChanged = optionsValues.reduce((acc, el, index) => {
      return acc || (el.name !== this.#optionsValues[index]?.name) || (el.value !== this.#optionsValues[index]?.value);
    }, false);

    this.#variantId = variantId;
    this.#optionsValues = optionsValues;

    const isAvailabilityChanged = (areOptionsChanged || this.#isHydration) ? this.#updateAvailability() : false;
  ...
  async #render(event) {
    ...
    if (!event.detail.isVariantChanged || this.#isHydration)
      return;
    ...
    if (this.#data.section.updateURL) {
      window.history.replaceState({ }, '', `${ this.#data.section.requestURL }${ this.#variantId > 0 ? `?variant=${ this.#variantId }` : '' }`);
    }

    const url = `${ this.#data.section.requestURL }?${ this.#variantId > 0 ? `variant=${ this.#variantId }&` : '' }section_id=${ this.#data.section.sectionId }`
  ...
  #updateAvailability() {
    ...
    this.#data.product.variants.forEach(variant => {
      for (let i = 0; i < this.#optionsHierarchy.length; i++) {
        ...
        if (variant.options[optionIndex] !== this.#optionsValues[optionIndex].value) {
          break;
        }
      }
    });
    ...
    for (let i = 0; i < newAvailabilityKeys.length; i++) {
      ...
      if (!(optionProp in prevAvailability))
        return true;

      const newValuesKeys = Object.keys(newAvailability[optionProp]);
      if (newValuesKeys.length !== Object.keys(prevAvailability[optionProp]).length)
        return true;

      for (let j = 0; j < newValuesKeys.length; j++) {
        const valueProp = newValuesKeys[j];
        if (newAvailability[optionProp][valueProp] !== prevAvailability[optionProp][valueProp])
          return true;
      }
    }
