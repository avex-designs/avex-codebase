...
/*!**************************************************!*\
  !*** ./src/scripts/components/product-option.js ***!
  \**************************************************/
const attributes = {
...
class ProductOption extends HTMLElement {
  ...
  #render() {
    ...
    const option = state.optionsValues.find(option => option.name === this.#optionName);
      ...
      this.querySelectorAll('input[type="radio"]').forEach($input => {
        ...
        if (availability) {
          if (!($input.value in availability))
            ...
            status = availabilityStatuses.NOT_AVAILABLE;
        }
